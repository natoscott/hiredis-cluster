...
void test_multicluster() {
    ...
    status = redisClusterConnect2(cc1);
    ASSERT_MSG(status == REDIS_OK, cc1->errstr);
    ...
    status = redisClusterConnect2(cc2);
    ASSERT_MSG(status == REDIS_OK, cc2->errstr);
...
void test_async_password_ok() {
    ...
    status =
        redisClusterAsyncCommand(acc, commandCallback, &r, "SET key1 Hello");
    assert(status == REDIS_OK);
...
void test_async_multicluster() {
    ...
    // Connect to first cluster
    status = redisClusterConnect2(acc1->cc);
    assert(status == REDIS_OK);
    assert(acc1->err == 0);
    assert(acc1->cc->err == 0);

    // Connect to second cluster
    status = redisClusterConnect2(acc2->cc);
    assert(status == REDIS_OK);
    assert(acc2->err == 0);
    ...
    status =
        redisClusterAsyncCommand(acc1, commandCallback, &r1, "SET key Hello1");
    assert(status == REDIS_OK);

    ExpectedResult r2 = {.type = REDIS_REPLY_STATUS, .str = "OK"};
    status =
        redisClusterAsyncCommand(acc2, commandCallback, &r2, "SET key Hello2");
    assert(status == REDIS_OK);

    // Verify key in first cluster
    ExpectedResult r3 = {.type = REDIS_REPLY_STRING, .str = "Hello1"};
    status = redisClusterAsyncCommand(acc1, commandCallback, &r3, "GET key");
    assert(status == REDIS_OK);

    // Verify key in second cluster and disconnect
    ExpectedResult r4 = {
        .type = REDIS_REPLY_STRING, .str = "Hello2", .disconnect = true};
    status = redisClusterAsyncCommand(acc2, commandCallback, &r4, "GET key");
    assert(status == REDIS_OK);

    // Verify that key is still accessible in connected cluster
    ExpectedResult r5 = {
        .type = REDIS_REPLY_STRING, .str = "Hello1", .disconnect = true};
    status = redisClusterAsyncCommand(acc1, commandCallback, &r5, "GET key");
    assert(status == REDIS_OK);
